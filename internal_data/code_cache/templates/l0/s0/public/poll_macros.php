			...
			foreach ($__vars['poll']['Responses'] AS $__vars['response']) {
				...
				if ($__templater->method($__vars['poll'], 'canEditDetails', array())) {
					$__compilerTemp1 .= '
						...
						'aria-label' => $__templater->filter('Poll choice', array(array('for_attr', array()),), false),
						'placeholder' => $__templater->filter('Poll choice' . $__vars['xf']['language']['ellipsis'], array(array('for_attr', array()),), false),
					)) . '
						';
				} else {
					$__compilerTemp1 .= '

...
if(e.css("transition-property").match(/(^|\s|,)-xf-(width|height)($|\s|,)/)){t=RegExp.$2;var x=f[t],u=e.css(x),y=u[t],v="transition."+t,w=e.data(v),z=n.style,B=z.transition||z["-webkit-transition"]||z["-moz-transition"]||z["-o-transition"]||"",A;if(void 0===w)for(w={},A=0;A<x.length;A++)w[x[A]]=z[x[A]]||"";if(0==e[t]())for(A in y="0",u)u.hasOwnProperty(A)&&(u[A]="0");e.data(v,w).css("transition","none")[l](d);x=e.css(x);w=x[t];if(0==e[t]())for(A in w="0",x)x.hasOwnProperty(A)&&(x[A]="0");e[p](d);
if(y!=w){var C=k;e.css(u);n.offsetWidth;e.css("transition",B).css(x);k=function(){e.css(e.data(v)).removeData(v);C&&C.apply(this,arguments)}}else e.css("transition",B)}e.onTransitionEnd(m,function(){e.removeClass("is-transitioning");k&&k.apply(this,arguments)});e[l](h)}else t()}var f={height:"height padding-top padding-bottom margin-top margin-bottom border-top-width border-bottom-width".split(" "),width:"width padding-left padding-right margin-left margin-right border-right-width border-left-width".split(" ")};
...
!0}else p=!0;p&&(d.push(e[2]),h=h.replace(e[0],""))}h=h.replace(/<noscript>([\s\S]*?)<\/noscript>/ig,"")}var n=g(k?g.parseHTML(h,null,m):h);n.retinaFix();n.find("noscript").empty().remove();if(b instanceof g){var t=b;b=function(u){t.html(u)}}if("function"!==typeof b)console.error("onReady was not a function");else{var x=!1;h=function(u){if(!x){x=!0;for(var y=0;y<d.length;y++)g.globalEval(d[y]);a.jsState&&(XF.config.jsState=XF.applyJsState(XF.config.jsState,a.jsState));u||XF.activate(n)}};!1!==b(n,
...
return f[b]}return XF._measureScrollBar[b]},windowHeight:function(){return XF.browser.ios||XF.browser.android?q.innerHeight:g(q).height()},pageLoadScrollFix:function(){if(q.location.hash){var a=!1,b=function(){if(!a){var c=q.location.hash.replace(/[^a-zA-Z0-9_-]/g,"");c=c?g("#"+c):g();c.length&&c.get(0).scrollIntoView(!0)}};"complete"==r.readyState?setTimeout(b,0):(setTimeout(function(){g(q).one("scroll",function(c){a=!0})},100),g(q).one("load",b))}},applyJsState:function(a,b){a=a||{};if(!b)return a;
for(var c in b)b.hasOwnProperty(c)&&!a[c]&&XF.jsStates.hasOwnProperty(c)&&XF.jsStates[c]()&&(a[c]=!0);return a},jsStates:{facebook:function(){return this.fbSdk()},fbSdk:function(){g(r.body).append(g('<div id="fb-root" />'));q.fbAsyncInit=function(){FB.init({version:"v2.7",xfbml:!0})};XF.loadScript("https://connect.facebook.net/"+XF.getLocale()+"/sdk.js");return!0},twitter:function(){q.twttr=function(){var a=q.twttr||{};XF.loadScript("https://platform.twitter.com/widgets.js")&&(a._e=[],a.ready=function(b){a._e.push(b)});
return a}();return!0},flickr:function(){XF.loadScript("https://embedr.flickr.com/assets/client-code.js");return!0},instagram:function(){XF.loadScript("https://platform.instagram.com/"+XF.getLocale()+"/embeds.js",function(){g(r).on("xf:reinit",function(a,b){q.instgrm&&instgrm.Embeds.process(b instanceof g?b.get(0):b)})})},reddit:function(){XF.loadScript("https://embed.redditmedia.com/widgets/platform.js");XF.loadScript("https://www.redditstatic.com/comment-embed.js",function(){g(r).on("xf:reinit",
function(a,b){q.rembeddit&&rembeddit.init()})});return!0},reddit_comment:function(){return this.reddit()},imgur:function(){q.imgurEmbed||(q.imgurEmbed={tasks:g("blockquote.imgur-embed-pub").length});XF.loadScript("//s.imgur.com/min/embed-controller.js",function(){g(r).on("xf:reinit",function(a,b){imgurEmbed.tasks+=g("blockquote.imgur-embed-pub").length;for(a=0;a<imgurEmbed.tasks;a++)imgurEmbed.createIframe(),imgurEmbed.tasks--})});return!0},pinterest:function(){XF.loadScript("//assets.pinterest.com/js/pinit.js",
...
a),!1):b},XF.Cookie={get:function(a){return(a=(new RegExp("(^| )"+XF.config.cookie.prefix+a+"=([^;]+)(;|$)")).exec(r.cookie))?decodeURIComponent(a[2]):null},getEncodedCookieValue(a,b,c,f){var e=XF.config.cookie;return e.prefix+a+"="+encodeURIComponent(b)+(void 0===c?"":";expires="+c.toUTCString())+(e.path?";path="+e.path:"")+(e.domain?";domain="+e.domain:"")+(f?";samesite="+f:"")+(e.secure?";secure":"")},getEncodedCookieValueSize:function(a,b,c,f){return this.getEncodedCookieValue(a,b,c,f).length},
set:function(a,b,c,f){r.cookie=this.getEncodedCookieValue(a,b,c,f)},getJson:function(a){a=this.get(a);if(!a)return{};try{return g.parseJSON(a)||{}}catch(b){return{}}},setJson:function(a,b,c){this.set(a,JSON.stringify(b),c)},remove:function(a){var b=XF.config.cookie;r.cookie=b.prefix+a+"="+(b.path?"; path="+b.path:"")+(b.domain?"; domain="+b.domain:"")+(b.secure?"; secure":"")+"; expires=Thu, 01-Jan-70 00:00:01 GMT"},supportsExpiryDate:function(){return!0}},XF.LocalStorage={getKeyName:function(a){return XF.config.cookie.prefix+
...
0,e,d=null,h=function(){g.ajax({url:XF.canonicalizeUrl("job.php"),type:"post",cache:!1,dataType:"json",global:!1}).always(function(u){u&&u.more&&setTimeout(h,100)})},k=function(u){e=XF.ajax("post",XF.canonicalizeUrl("job.php"),{only_ids:u},function(y){y.more&&y.ids&&y.ids.length?(y.status&&g("#xfAutoBlockingJobStatus").text(y.status),setTimeout(function(){k(y.ids)},0)):(m(),y.moreAuto&&setTimeout(h,100))},{skipDefault:!0}).fail(m)},m=function(){d&&d.hide();f--;0>f&&(f=0);0==f&&(g(r).trigger("job:auto-blocking-complete"),
t());e&&e.abort();e=null},l=function(){c&&c.hide();a=!1;g(r).trigger("job:manual-complete");t();b&&b.abort();b=null},p=function(){c||(c=n("xfManualJobStatus"));return c},n=function(u){u=XF.getOverlayHtml({title:XF.phrase("processing..."),dismissible:!1,html:'<div class="blockMessage"><span id="'+u+'">'+XF.phrase("processing...")+"</span></div>"});return new XF.Overlay(u,{backdropClose:!1,keyboard:!1})},t=function(){x()||g(r).trigger("job:blocking-complete")},x=function(){return a||0<f};return{isBlockingJobRunning:x,
runAuto:h,runAutoBlocking:function(u,y){if("number"===typeof u)u=[u];else if(!Array.isArray(u))return;u.length&&(f++,d||(d=n("xfAutoBlockingJobStatus")),d.show(),y||(y=XF.phrase("processing...")),g("#xfAutoBlockingJobStatus").text(y),k(u))},runManual:function(u){var y=XF.config.job.manualUrl;if(y){if(null===u)v=null;else{var v=v||[];"number"===typeof u?v.push(u):Array.isArray(u)&&v.push.apply(v,u)}if(!a){a=!0;p().show();var w=function(B){b=XF.ajax("post",y,B?{only_id:B}:null,function(A){A.jobRunner?
(g("#xfManualJobStatus").text(A.jobRunner.status||XF.phrase("processing...")),setTimeout(function(){w(B)},0)):z()},{skipDefault:!0}).fail(l)},z=function(){Array.isArray(v)&&0==v.length?l():w(v?v.shift():null)};z()}}},stopManual:l,getManualOverlay:p}}(),XF.Loader=function(){var a=XF.config.css,b=XF.config.js,c=function(f,e,d){f=f||[];e=e||[];var h=[],k=[],m;for(m=0;m<f.length;m++)b.hasOwnProperty(f[m])||h.push(f[m]);for(m=0;m<e.length;m++)a.hasOwnProperty(e[m])||k.push(e[m]);var l=(h.length?1:0)+(k.length?
1:0),p=function(){l--;0==l&&d&&d()};l?(h.length&&XF.loadScripts(h,function(){g.each(h,function(n,t){b[t]=!0});p()}),k.length&&((f=XF.config.url.css)?(f=f.replace("__SENTINEL__",k.join(",")),g.ajax({type:"GET",url:f,cache:!0,global:!1,dataType:"text",success:function(n){var t=XF.config.url.basePath;t&&(n=n.replace(/(url\(("|')?)([^"')]+)(("|')?\))/gi,function(x,u,y,v,w,z){v.match(/^([a-z]+:|\/)/i)||(v=t+v);return u+v+w}));g("<style>"+n+"</style>").appendTo("head")}}).always(function(){g.each(k,function(n,
...
"xf:action-start":c,ajaxStop:f,"xf:action-stop":f})},show:c,hide:f}}(),XF.DynamicDate=function(){var a,b,c,f,e,d,h=!1,k,m=function(){k=setInterval(function(){l(r)},2E4)},l=function(n){h||this.initialize();n=g(n).find("time[data-time]");var t=n.length,x=Math.floor((new Date).getTime()/1E3)-a,u=new Date;u.setHours(0,0,0,0);b+x>e&&(u.getDay(),e=p(u,1),c=p(u,0),f=p(u,-1),d=p(u,-6));for(var y=0;y<t;y++){var v=n[y];var w=g(v);var z=parseInt(v.getAttribute("data-time"),10);var B=b-z+x;var A=v.xfDynType;
-2>B?(B=z-(b+x),60>B?"futureMoment"!=A&&(w.text(XF.phrase("in_a_moment")),v.xfDynType="futureMoment"):120>B?"futureMinute"!=A&&(w.text(XF.phrase("in_a_minute")),v.xfDynType="futureMinute"):3600>B?(z=Math.floor(B/60),A!=="futureMinutes"+z&&(w.text(XF.phrase("in_x_minutes",{"{minutes}":z})),v.xfDynType="futureMinutes"+z)):z<e?"latertoday"!=A&&(w.text(XF.phrase("later_today_at_x",{"{time}":w.attr("data-time-string")})),v.xfDynType="latertoday"):z<p(u,2)?"tomorrow"!=A&&(w.text(XF.phrase("tomorrow_at_x",
{"{time}":w.attr("data-time-string")})),v.xfDynType="tomorrow"):(604800>B||(w.attr("data-full-date")?w.text(XF.phrase("date_x_at_time_y",{"{date}":w.attr("data-date-string"),"{time}":w.attr("data-time-string")})):w.text(w.attr("data-date-string"))),v.xfDynType="future")):60>=B?"moment"!==A&&(w.text(XF.phrase("a_moment_ago")),v.xfDynType="moment"):120>=B?"minute"!==A&&(w.text(XF.phrase("one_minute_ago")),v.xfDynType="minute"):3600>B?(z=Math.floor(B/60),A!=="minutes"+z&&(w.text(XF.phrase("x_minutes_ago",
{"{minutes}":z})),v.xfDynType="minutes"+z)):z>=c?"today"!==A&&(w.text(XF.phrase("today_at_x",{"{time}":w.attr("data-time-string")})),v.xfDynType="today"):z>=f?"yesterday"!==A&&(w.text(XF.phrase("yesterday_at_x",{"{time}":w.attr("data-time-string")})),v.xfDynType="yesterday"):z>=d?"week"!==A&&(w.text(XF.phrase("day_x_at_time_y",{"{day}":XF.phrase("day"+(new Date(1E3*z)).getDay()),"{time}":w.attr("data-time-string")})),v.xfDynType="week"):"old"!==A&&(w.attr("data-full-date")?w.text(XF.phrase("date_x_at_time_y",
{"{date}":w.attr("data-date-string"),"{time}":w.attr("data-time-string")})):w.text(w.attr("data-date-string")),v.xfDynType="old")}},p=function(n,t){return Math.floor((new Date(n.valueOf())).setFullYear(n.getFullYear(),n.getMonth(),n.getDate()+t)/1E3)};return{initialize:function(){if(!h){h=!0;var n=XF.config.time;a=XF.getLocalLoadTime();b=n.now;c=n.today;f=n.yesterday;e=n.tomorrow;d=n.week;void 0!==r.hidden?(r.hidden||m(),g(r).on("visibilitychange",function(){r.hidden?clearInterval(k):(m(),l(r))})):
...
keydown:function(c){switch(c.key){case "Tab":case "Enter":a(!0)}}})},toggle:a,isKeyboardNav:function(){return b}}}(),XF.ScrollButtons=function(){function a(w){if(!l){w=q.pageYOffset||r.documentElement.scrollTop;var z=t;t=w;if(w>z)"down"!=x&&(x="down",u=z);else if(w<z)"up"!=x&&(x="up",u=z);else return;if(p){if("up"!==x||100>t){y&&(y.cancel(),y=null);return}if(30>u-w)return}y||(y=XF.requestAnimationTimeout(function(){y=null;b();f()},200))}}function b(){n||(v.addClassTransitioned("is-active"),n=!0)}
function c(){n&&(v.removeClassTransitioned("is-active"),n=!1)}function f(){e();m=setTimeout(function(){c()},3E3)}function e(){clearTimeout(m)}function d(){e();b()}function h(){e()}function k(){l=!0;setTimeout(function(){l=!1},500);c()}var m=null,l=!1,p=!1,n=!1,t=q.pageYOffset||r.documentElement.scrollTop,x=null,u=null,y,v=null;return{initialize:function(){if(v&&v.length)return!1;v=g(".js-scrollButtons");if(!v.length)return!1;"up"===v.data("trigger-type")&&(p=!0);v.on({"mouseenter focus":d,"mouseleave blur":h,
...
k)).appendTo(this.$results)}if(this.$results.children().length){this.selectResult(0,!0);this.headerHtml&&(h=g("<li />").addClass("menu-header menu-header--small").attr("unselectable","on").html(this.headerHtml),h.prependTo(this.$results));if(!this.resizeBound)g(q).onPassive("resize",XF.proxy(this,"hideResults"));this.$results.css({top:"",left:"",right:"",bottom:""});var x=this.$results,u=function(y){g.isFunction(y)&&(y=y(x,c));if(!y){var v=c.offset();y={top:v.top+c.outerHeight(),left:v.left};XF.isRtl()&&
(y.right=g("html").width()-v.left-c.outerWidth(),y.left="auto")}return y};this.stopScrollWatching();(b=c.parents().filter(function(){switch(g(this).css("overflow-x")){case "scroll":case "auto":return!0;default:return!1}}))&&b.length&&(b.on("scroll.autocomplete",function(){x.css(u(f))}),this.$scrollWatchers=b);this.$results.css(u(f)).show();this.resultsVisible=!0}}else this.hideResults()},resultClick:function(a){a.stopPropagation();this.insertResult(this.getResultText(a.currentTarget),a.currentTarget,
...
(c=this.getResultText(b),this.options.beforeInsert&&(c=this.options.beforeInsert(c,b)),this.insertResult(c,b,a),c=!0);this.hideResults();return c},insertResult:function(a,b,c){if(this.options.onInsert)this.options.onInsert(a,b,c)},getResultText:function(a){switch(this.options.insertMode){case "text":var b=g(a).data("insert-text");break;case "html":b=g(a).data("insert-html")}return b}}),XF.AutoCompleter=XF.create({options:{url:null,method:"GET",idleWait:200,minLength:2,at:"@",keepAt:!0,insertMode:"text",
displayTemplate:"{{{icon}}}{{{text}}}",beforeInsert:null},$input:null,ed:null,results:null,visible:!1,idleTimer:null,pendingQuery:"",__construct:function(a,b,c){this.options=g.extend(!0,{},this.options,b);this.$input=a;this.ed=c;this.options.url?(("string"!=typeof this.options.at||1<this.options.at.length)&&console.error("The 'at' option should be a single character string."),this.init()):console.error("No URL option passed in to XF.AutoCompleter.")},init:function(){var a=this,b={onInsert:function(c){a.insertResult(c)},
beforeInsert:this.options.beforeInsert,insertMode:this.options.insertMode,displayTemplate:this.options.displayTemplate};this.ed&&(b.clickAttacher=function(c,f){a.ed.events.bindClick(c,c,f)});this.results=new XF.AutoCompleteResults(b);this.ed?(this.ed.events.on("keydown",XF.proxy(this,"keydown"),!0),this.ed.events.on("keyup",XF.proxy(this,"keyup"),!0),this.ed.events.on("click blur",XF.proxy(this,"blur")),g(this.ed.$wp).onPassive("scroll",XF.proxy(this,"blur"))):(this.$input.on("keydown",XF.proxy(this,
"keydown")),this.$input.on("keyup",XF.proxy(this,"keyup")),this.$input.on("click blur",XF.proxy(this,"blur")),g(r).onPassive("scroll",XF.proxy(this,"blur")))},keydown:function(a){if(this.visible)switch(a.which){case 40:return this.results.selectResult(1),a.preventDefault(),!1;case 38:return this.results.selectResult(-1),a.preventDefault(),!1;case 27:return this.hide(),a.preventDefault(),!1;case 13:if(this.visible)return this.results.insertSelectedResult(a),a.preventDefault(),!1}},keyup:function(a){if(this.visible)switch(a.which){case 40:case 38:case 13:return}this.hide();
...
this.$input;b.autofocus();a=b.getSelection();if(!a||1>=a.end)return!1;b=b.val().substring(0,a.end)}var f=b.lastIndexOf(this.options.at);if(-1===f)return null;if(0===f||b.substr(f-1,1).match(/(\s|[\](,]|--)/)){var e=b.substr(f+1);if(!e.match(/\s/)||15>=e.length)return{text:b,textNode:c,start:f,query:e.replace(new RegExp(String.fromCharCode(160),"g")," "),range:a}}return null},foundMatch:function(a){this.pendingQuery!==a&&(this.pendingQuery=a,a.length>=this.options.minLength&&"["!==a.substr(0,1)&&this.getPendingQueryOptions())},
...
b.textNode;var e=f.nodeValue,d=f.splitText(this.options.keepAt?c:c-1);d.textContent=e.substr(c+b.query.length);a="html"===this.options.insertMode?g.parseHTML(a+"\u00a0"):r.createTextNode(a+"\u00a0");g(d).before(a);f.parentNode.normalize();this.ed.selection.restore()}else d=this.$input,d.autofocus(),-1!==c&&(d.setSelection(b.start,f.end),d.replaceSelectedText((this.options.keepAt?this.options.at:"")+a+" ","collapseToEnd"))}},show:function(a,b){var c=this.getCurrentMatchInfo(),f=this.$input,e=f.dimensions(),
d=this;if(c)if(this.visible=!0,this.ed){var h=c.range;this.results.showResults(a,b,f,function(k){if(!h||!h.getBoundingClientRect)return k=h.startContainer,{top:(3===k.nodeType?g(k.parentNode):g(k)).dimensions().bottom+3,left:e.left+5};var m=h.cloneRange();m.setStart(c.textNode,c.start);m.setEnd(c.textNode,c.start+1);m=m.getBoundingClientRect();return d.getResultPositionForSelection(m.left,m.bottom,h.getBoundingClientRect().left,k,e)})}else this.results.showResults(a,b,f,function(k){for(var m=g("<div />"),
l=q.getComputedStyle(f[0]),p,n="",t=0;t<l.length;t++)p=l[t],n+=p+": "+l.getPropertyValue(p)+"; ";m[0].style.cssText=n;m.css({position:"absolute",height:"",width:f.outerWidth(),opacity:0,top:0,left:"-9999px"});m[0].textContent=f.val();m.appendTo(r.body);n=r.createRange();n.setStart(m[0].firstChild,c.start);n.setEnd(m[0].firstChild,c.start+1);t=n.getBoundingClientRect();var x=m.dimensions();l=e.left+(t.left-x.left);p=e.top+(t.bottom-x.top);n.setStart(m[0].firstChild,c.start+1+c.query.length);n.setEnd(m[0].firstChild,
